...
void DutVcsBase::init(int argc, char **argv)
{
    ...
    // set VCS cycle to 0
    VcsInit();
    svSetScope(svGetScopeFromName("ICache_top"));
...
void DutVerilatorBase::init(int argc, char **argv)
{
    ...
    // create top module
    this->top = new VICache {contextp};

    svSetScope(svGetScopeFromName("TOP.ICache_top"));
...
int DutVerilatorBase::Step(uint64_t ncycle, bool dump)
{
    ...
    if (dump) {
        for (int i = 0; i < ncycle; i++) {
            ((VICache *)(top))->eval();
            ((VICache *)(top))->contextp()->timeInc(1);
        }
    } else {
        assert(ncycle == 1);
        ((VICache *)(top))->eval_step();
    }
    ...
    if (this->top != nullptr) {
        ...
        if (this->coverage_file_path.size() > 0)
            ...
            contextp->coveragep()->write(
                "VICache_coverage.dat");
#endif
        ((VICache *)(this->top))->final();
        delete (VICache *)(this->top);
        delete contextp;
        this->top = nullptr;
    }
    return 0;
...
{
#if defined(VL_TRACE)
    ((VICache *)(this->top))->contextp()->dumpfile(filename);
    ((VICache *)(this->top))->rootp->vlSymsp->_traceDumpOpen();
#else
...
uint64_t DutUnifiedBase::GetDPIHandle(char *name, int towards)
{
    ...
    if (towards == 0) {
        sprintf(func_name, "get_%sxxPfBDHPgM7CX", name);
    } else if (towards == 1) {
        sprintf(func_name, "set_%sxxPfBDHPgM7CX", name);
    } else if (towards == -1) {
